...
def main():
    ...
    if args.dataset_name is not None:
        ...
        json_path = os.path.join(data_folder, "dpo_data.json")
        image_folder = os.path.join(data_folder, "images_dpo")
    ...
    if args.train_method == 'dpo':
        ...
        def preprocess_train(examples):
            ...
            for col_name in ['jpg_0', 'jpg_1']:
                ...
                images = [Image.open(os.path.join(image_folder, image)).convert("RGB")
                            for image in examples[col_name]]
                pixel_values = [train_transforms(image) for image in images]
            ...
            if accelerator.sync_gradients:
                ...
                if global_step % args.checkpointing_steps == 0:
                    if accelerator.is_main_process:
                        ...
                        del_step = global_step - 2*args.checkpointing_steps
                        del_path = os.path.join(args.output_dir, f"checkpoint-{del_step}")
                        if os.path.exists(del_path):
                            shutil.rmtree(del_path)
                            logger.info(f"Removed old checkpoint: {del_step}")
